...
st.set_page_config(page_title="FreshPredict", layout="wide")

# Toned down the "AI" hype. Focus on professionalism.
st.title("🌿 FreshPredict: Floral Supply Chain Twin")
st.markdown("### Predictive Analytics & Logistics Optimization")
st.markdown("---")

# 2. Load the Data
df = pd.read_csv("data.csv")

# 3. Data Prep & Model Training
...
# 6. Top Row: Core Metrics
st.header("📊 Vase-Life Prediction")
...
# 7. Middle Row: Solutions & Routing (No Graphs, Just Value)
col_route, col_opt = st.columns(2)

with col_route:
    st.header("🚚 Routing Recommendation")
    if remaining_life < 4:
        st.error("**URGENT ACTION: ROUTE TO LOCAL FLORISTS.**\nProduct will not survive export timeline. Reroute domestically to prevent total loss of investment.")
    elif remaining_life >= 4 and remaining_life < 8:
        st.warning("**STANDARD ROUTING:**\nSafe for regional EU distribution. Monitor cold chain logistics strictly.")
    else:
        st.success("**APPROVED FOR EXPORT:**\nOptimal vase life secured. Cleared for long-haul international shipping.")

with col_opt:
    st.header("⚡ Logistics Optimization")
    st.write("System simulated solutions to recover vase life:")
    
    # Simulate dropping the transport temperature by 2 degrees
    ...
    if days_gained > 0.1:
        st.success(f"**Cooling Adjustment:** Dropping transport temperature by 2°C will recover **+{days_gained:.1f} days** of quality.")
    else:
        st.info("**Cooling Adjustment:** Current transport temperature is highly optimized.")
        
    # Simulate speeding up the truck by 1 day
    ...
    if days_gained2 > 0.1:
        st.warning(f"**Transit Adjustment:** Expediting shipping by 1 day preserves **+{days_gained2:.1f} days** of retail quality.")

st.markdown("---")

# 8. Bottom Row: Clean, Error-Free Database
st.header("📋 Database View")
st.write("Click any column header to sort.")
# Removed the matplotlib styling to prevent cloud errors. Standard, clean table.
st.dataframe(df.drop(columns=['Flower_Code']), use_container_width=True)